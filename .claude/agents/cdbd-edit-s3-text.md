...
- [ ] **줄마다 스타일이 다른 줄바꿈은 S3 ❌ → F1 소관** — 제목=Bold 큰 글씨 / 일시=작고 연한 글씨처럼 줄별 색·웨이트·사이즈가 다르면 단순 linebreak로 못 담음 → **F1이 content를 줄별 text 노드+inline 스타일로 원자적 재작성**(줄바꿈+디자인 동시). S3는 **같은 스타일 다줄**만 담당(2026-07-07 5025 세션 일시).
