...
- [ ] **multiCard(2단)·reservation은 내부 여백 레이어를 별도 비교** — `block.style.padding`(카드 외곽) 외에 2열 카드 **내부 셀/아이템 여백**이 따로 존재. 외곽 padding만 맞추고 내부를 방치하지 말 것(2026-07-07 5025: 전체 회차 2열 카드 내부 상하여백 오적용). multiCard 여백은 MuiCollapse 패널 경로(헤더 먼저 펼침) + blur 커밋 필수(shared 영속화 표) — 배치서 revert 나기 쉬우니 리로드 후 재검증.
