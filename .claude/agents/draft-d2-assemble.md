...
description: CdBd 시안 파이프라인 D2 — 콘텐츠 팩 + 레이아웃 스타일(미니멀/에디토리얼/볼드)을 받아 해당 서랍에서 섹션을 골라 「조합 테스트」 페이지에 복제·조립하고 텍스트·이미지를 주입. 디폴트 테마 그대로 둔다(색·폰트는 D4, 통일은 D3). 원본 섹션 수정 절대 금지.
---
...
레시피의 **섹션 목적마다 그 스타일 서랍에서 섹션 1개를 골라** 복제하고, `3322:70` 「조합 테스트」에 폭 380 세로 프레임으로 조립한 뒤 콘텐츠 팩을 주입한다.
...
### 1. 섹션 선택 — 🎲 「거른 뒤 랜덤」 (2026-09-07 이선호 확정 · 필수)

> 🔴 **왜 바뀌었나**: 「조건에 맞는 걸 고른다」로 두면 **매번 같은 걸 집는다.** 75안 실측 = `contact-mixed-01` 하나가 **66/75안(88%)**, `intro-minimal-01` 46안, `list-minimal-01` 23안(나열 서랍은 46개인데도). 「문의는 3안 다르게」 규칙은 이미 있었는데 안 지켜졌다 → **선택을 자동화한다.**

```
후보 = 서랍에서 [슬롯이 이 콘텐츠에 맞는 것]만 추림      ← 아래 「맞는지」 기준으로 먼저 거른다
if 후보 ≤ 3개:  → 3안 공용 (다양성 면제 · form 2·location 6)
else:           → 랜덤 pick, 이번 3안에서 이미 뽑힌 건 제외
                  (히어로·헤더는 직전 배치 것도 제외)
```

- **① 거르기가 랜덤보다 먼저** — 서랍 전체가 아니라 **내용에 맞는 후보** 안에서만 랜덤. 이미지가 없는데 이미지 슬롯 3개짜리가 뽑히면 다양성은 늘고 페이지는 망가진다.
- **② 비복원** — 안1이 쓴 섹션은 안2·안3의 같은 서랍에서 **뺀다**(안 하면 소개 서랍 12개 기준 **24%** 확률로 두 안이 같아짐).
- **③ 이력 제외는 히어로·헤더·팔레트만** — 전 섹션에 걸면 서랍이 마른다. 유저는 **위 1.5화면**으로 판단한다.
- **④ 얇은 서랍은 면제** — `form`(2)·`location`(6)은 **3안 공용 OK**. 억지 차별화는 완성도만 깎는다. 필요하면 섹션이 아니라 **카드 배경·테두리·정렬·타이틀**로.
- **🌱 난수 시드를 `picks[].seed`에 기록** — 같은 시안 재현용.
- 풀버전 = [[4-5. 섹션 라이브러리#🎲 섹션 선택 로직 — 「서랍에서 랜덤으로 뽑는다」 (2026-09-07 이선호 확정 · 필수)]]

### 1-B. 후보를 거르는 기준 — 🚨 이름·크기로 찍지 말 것

> 🔴 실패 사례(2026-07-16): 이름과 높이만 보고 골랐더니 **나열 = 리뷰(★★★★★) 섹션**, **핵심 정보 = 통계 숫자 카드**이 나왔다. 프로그램·일시장소와 전혀 안 맞아 통째로 다시 골랐다.
...
콘텐츠 팩 `items`가 섹션 항목 수보다 많으면 **마지막 항목을 clone**해서 개수를 맞춘다.
...
🔴 실패: A① 3안이 **순서가 완전히 동일**(히어로·스토리·핵심 정보·나열·위치·문의·푸터)했다 → 축 하나를 통째로 못 씀.
...
{ "style":"미니멀", "frameId":"3390:91",
  ...
  "variationAxes": ["도입=텍스트만","이미지비중=0","순서=핵심 정보 상단"] }
```
