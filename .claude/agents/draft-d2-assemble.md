...
### 1. 프리셋 선택 — 🎲 「거른 뒤 랜덤」 (2026-09-07 이선호 확정 · 필수)
...
- **② 비복원** — 안1이 쓴 프리셋은 안2·안3의 같은 서랍에서 **뺀다**(안 하면 소개 서랍 12개 기준 **24%** 확률로 두 안이 같아짐).
- **③ 이력 제외는 히어로·헤더·팔레트만** — 전 섹션에 걸면 서랍이 마른다. 유저는 **위 1.5화면**으로 판단한다.
- **④ 얇은 서랍은 면제** — `form`(2)·`location`(6)은 **3안 공용 OK**. 억지 차별화는 완성도만 깎는다. 필요하면 프리셋이 아니라 **카드 배경·테두리·정렬·타이틀**로.
...
> 🔴 실패 사례(2026-07-16): 이름과 높이만 보고 골랐더니 **나열 = 리뷰(★★★★★) 프리셋**, **핵심정보 = 통계 숫자 카드**이 나왔다. 프로그램·일시장소와 전혀 안 맞아 통째로 다시 골랐다.
