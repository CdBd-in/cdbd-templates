...
- 🧱 **헤더를 쓰면 무조건 페이지와 '구분'돼 보이게 (필수, 2026-07-24 이선호)** — **하단 구분선**(`{텍스트색}`×20%~솔리드) **또는 배너 배경색**(`{버튼색}`×8%(다크 10%)/솔리드 · `{텍스트색}` 솔리드) **둘 중 하나 필수**. 헤더 배경 = 페이지 배경색(구분 요소 0)이면 ❌. ⚠️ **다크 테마**는 헤더가 페이지색과 같아지기 쉬움 → 구분선 `{텍스트색}`×**30%**(다크 보정). 프리셋 원본 헤더는 전부 둘 중 하나를 가짐 → D4 무드 적용 후에도 보존.
