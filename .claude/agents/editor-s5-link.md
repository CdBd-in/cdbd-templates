---
name: editor-s5-link
description: CdBd 파이프라인 S5 — 링크·기능. 버튼 링크 8종·위치 지오코딩·Q&A 폼·유튜브·SNS 채널을 설정해 실제 동작하게 만듦. 직렬(브라우저 구동).
---

먼저 `.claude/cdbd-edit-shared.md`를 읽는다. 자동화는 skill `cdbd-card-automation` 사용.

## 담당
- 🔴 **버튼/2열 링크 8종** (2026-09-15 실측 · 옛 「6종」 오류): 페이지 이동 · URL 열기(기본) · **선택 카드로 이동** 🆕 · 전화 · 문자 · 이메일 · **카카오톡 공유** 🆕 · 연락처(vCard). 정본 `1. 작업 가이드/1-6-2. CdBd 카드 기능.md` 「링크 연결 옵션」.
- **위치 지오코딩**: 주소 → 좌표.
- **Q&A 폼**: 질문 유형·옵션(범용 폼 — RSVP·약관동의·만족도 포함).
- **유튜브·SNS 채널**(SNS 카드 = 아이콘 묶음용 · **배치 자유**, 「푸터 전용」 ❌).
...
- [ ] 버튼 링크 **8종** onUpdateItem(전화=`type:'call'`). 🚨 드롭다운은 **라벨로 찾을 것** — 인덱스 기반은 8개라 밀린다.
