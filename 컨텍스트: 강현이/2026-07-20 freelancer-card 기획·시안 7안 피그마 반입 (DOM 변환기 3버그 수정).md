...
- **기획 문서**: [[../3. 신규 템플릿 기획/freelancer-card — 기획]] — 무드 **🎨 빈티지**(모던에서 변경). 프로필·명함 ②영업용 명함. 페르소나=1인 브랜드 디자이너 김도윤. 핵심 = **서비스 메뉴+가격 + 예산·일정 사전필터 Q&A**.
...
2. ~~채택안 선택~~ ✅ **1a 채택 확정 (2026-07-20)** — 아래 §"1a 채택 후 수정" 참조. 무드는 기획의 빈티지 → **프로페셔널로 변경**(사용자 결정).
3. **다음 = 3단계 마무리** — ①이미지 생성(gpt-image-1, 갤러리 4장 + 프로필 1장 · 현재 전부 회색 자리표시자) ②꼬리 잔여물(폰 하단 홈 인디케이터 선) 제거 ③4단계 CdBd 에디터 구현. ← **지금 여기**

---

## 🎯 1a 채택 후 수정 (2026-07-20)

**노드**: [`1355:193`](https://www.figma.com/design/oi8zIHLfy59O5zV8aysqq4/?node-id=1355-193) (27카드 · 3630px) · Q&A 카드 = `1355:336` · 신규 설명 카드 = `1359:71`

### ① 팔레트 교체 — coach-card 중복 해소
| 토큰 | 1a 원본 | coach-card | **신규** | 대비 |
|---|---|---|---|---|
| 배경 | `#141414` | `#0E0E10` | **`#12151C`** 잉크 네이비 | — |
| 텍스트 | `#ecece8` | `#F5F5F5` | **`#E8EAF0`** 쿨 오프화이트 | 15.2:1 ✅ |
| 포인트 | `#c8ff3c` | `#D7FF3E` | **`#4C8DFF`** 잉크 코발트 | 5.7:1 ✅ |

- 원본 3토큰이 coach-card와 **사실상 동일**했던 게 교체 사유(사용자 지적).
- 교체 후 **미매핑 hex 0개** — 페이지 전체에 3토큰만 존재(`#e8eaf0` 99 · `#4c8dff` 19 · `#12151c` 2). 채택 관문 「토큰 미매핑 색 = 0」 자동 검사 통과.
- ℹ️ 텍스트를 웜크림→쿨오프화이트로 옮긴 건 **에이전트 판단**(코발트·네이비와 톤 정합). 되돌리려면 `#ecece8`.

### ② 상담신청 폼 → Q&A 카드 규격 재작성
생성 시안이 **CdBd Q&A에 없는 요소 4개**를 그렸던 것을 교정 (528 → **902px**):

| 시안 원본 | CdBd 실제 | 조치 |
|---|---|---|
| 제목 밑 설명 부제 | 설명 슬롯 **없음** | 별도 텍스트 카드(`1359:71`)로 분리 |
| 예산 5·일정 4를 **알약 가로 배치** | 세로 목록 + 라디오 **고정** | 세로 라디오(마커 20 · gap 12 · 행간 14) |
| "예상 예산" 소형 라벨 | 라벨 자리 없음 | **질문 문장으로 승격** |
| 제목 좌정렬 | **중앙 고정** | 중앙 |

- 질문 간 간격 = **24**(CdBd 허용값 16/24/32/40 중), 제출 버튼 = **하단 내장·채움**(글자={배경색}) ✅
- 🚫 **필수 별표는 그리지 않음** — 카드 default라 시안 재현 대상 아님([[../1. 작업 가이드/1-6-2. CdBd 카드 기능]] §카드 default 기능).
- 라디오 마커는 **CdBd 표준 형태 그대로** 그림(모양 변경 ❌ · DESIGN.md §5).

### ③ SELECTED WORK 갤러리 → 넘겨보기 (2026-07-21)
- 그리드 6장 → **넘겨보기**로 변경(사용자 요청). 시안엔 **대표 슬라이드 1장(334×334 · 1:1 · r16)** 만, 화살표·도트는 **CdBd default라 미재현**.
- 카드 라벨에 설정 명시: `넘겨보기 6장 · 비율 1:1 · 모서리 둥근 · 내비게이션 ON · 자동넘김 OFF`.
- 판단(사용자 확인 필요할 수도): 내비게이션 **ON**(작업물 6개 직접 넘겨봐야 함) · 자동넘김 **OFF**(포트폴리오는 천천히). interior-portfolio(자동넘김 히어로)와 반대 — 여긴 탐색용이라.

### ④ 이미지 생성 (2026-07-21 · gpt-image-1 medium · 총 ~$0.30)
- **영구 보관**: `~/Desktop/freelancer-card/` (work1~6.png 1024² + profile.png)
- **톤**: 프로페셔널 뉴트럴 스튜디오 · 자연광 · 심플 도형 마크(글자 없음) · 다크 네이비 페이지 정합. 🎨 기획의 빈티지 종이질감은 무드 변경으로 미적용.
- **시안 반영 2장**: 프로필 아바타 `1355:196` ← profile(한국 남성 30대 초반 작업컷) · 갤러리 대표 슬라이드 `1363:23` ← work1(로고 목업).
- **4단계 업로드 대기 5장** (넘겨보기 나머지): work2=명함 / work3=패키지+쇼핑백 / work4=레터헤드·봉투 / work5=간판 사이니지 / work6=SNS 템플릿. 전부 `~/Desktop/freelancer-card/`.
- 업로드 방식: `upload_assets`(nodeId 없이) → imageHash → `use_figma`에서 fills 직접 IMAGE 설정 + 자리표시자 텍스트/임시프레임 제거 (auto-layout FRAME 함정 회피).
