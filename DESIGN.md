...
> 🔴 **2026-09-16 정정** — CdBd **텍스트 크기 슬라이더 = 12~60 선형** · **줄간격 슬라이더 = 1.2~2.0 · step 0.2(=20% 단위) · 5단계(1.2/1.4/1.6/1.8/2.0)**.
> ⚠️ 옛 서술 「줄간격 100·110·120…%(10% 단위)」는 **오류** — **100%(1.0)·110%는 슬라이더에 없다.** 입력란은 받지만 사용자가 UI를 만지면 스냅되므로 사용 ❌.
> Figma `100%` → CdBd **120**(R22 변환 규칙).
...
### Buttons
- **Shape:** **이미지 카드 = 8종**(각진·둥근·원형·아치·구름·꽃·반짝이·마름모) / **카드 배경·갤러리 = 각진(0px)·둥근(16px)·원형(999px) 3종**. 픽셀 임의값 ❌. *(2026-09-07 정정 — 옛 「3가지뿐」은 카드 배경 얘기였다)*
> 🔴 **2026-09-16 정정** — **모서리 종류는 카드마다 다르다. 일반화 금지.** 이미지 카드 **8종** · **지도(위치 안내)·프로필 이미지 = 원형/각진/둥근 3종** · 갤러리·유튜브·카드 배경 = 각진/둥근 2종.
> ⚠️ 옛 서술이 「지도」를 2종 쪽에 묶어둔 것은 **오류**(충돌 #6, 영상 2-9 실측 종결). 2026-09-07의 *「3종뿐이라 불가」* 오답도 이 혼동에서 나왔다.
...
### Cards / Containers
- **Corner:** 🔴 **카드마다 다름 — 일반화 금지**(2026-09-16). 이미지 **8종** · 프로필 이미지·지도 **3종(원형/각진/둥근)** · 갤러리·유튜브·**카드 배경 = 각진/둥근 2종**. 위 174행 참조.
...
- **Do** 모서리는 **카드별 옵션표를 확인**(카드 배경은 각진/둥근 2종), 여백은 0~40(2단위), 텍스트는 정해진 사이즈 스텝 안에서만 조정하라.
