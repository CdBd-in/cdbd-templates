...
> 색상·폰트 원본 라이브러리: [[1-3. 색상 팔레트]] · [[1-4. 폰트]] · 카드 스펙: [[1-6-2. CdBd 카드 기능]].
...
| 무드 | 배경 | 텍스트 | 버튼/액센트 | 대비 |
|---|---|---|---|---:|
| 🌿 내추럴 | `#F8F5F2` | `#232323` | `#078080` | 14.47 AAA |
| 🌸 페미닌 | `#DABAAF` | `#0B0C11` | `#CF2F89` | 10.81 AAA |
| ✨ 럭셔리 | `#004643` | `#FFFFFE` | `#F9BC60` | 10.72 AAA |
| 💼 프로페셔널 | `#F2F2F2` | `#000000` | `#0B64FE` | 18.76 AAA |
| 🌑 모던 | `#16161A` | `#FFFFFE` | `#7F5AF0` | 18.03 AAA |
| 🔥 비비드 | `#0F0E17` | `#FFFFFE` | `#FF8906` | 19.16 AAA |
| 🎨 빈티지 | `#F9F4EF` | `#020826` | `#8C7851` | 18.04 AAA |
| 🎉 플레이풀 | `#FFFFFE` | `#272343` | `#FFD803` | 14.91 AAA |
| 🌙 감성적 | `#FEF6E4` | `#001858` | `#F582AE` | 15.35 AAA |
...
**The No-Auto-Gap Rule.** CdBd엔 자동 분배 레이아웃(`SPACE_BETWEEN`/`SPACE_AROUND`)이 없다. 카드 간격은 각 카드의 상하 padding 또는 여백 카드로만. 가로 N등분은 등폭 FILL + 고정 간격(2열 카드).

## 5. Components

카드 시스템(기본 14종 + 2열 6종 · 📹 2026-09-04 실측). 각 카드는 페이지 프레임의 직계 자식(top-level)이며, 다른 속성·타입은 절대 한 카드로 묶지 않는다("평탄화").

### Buttons
- **Shape:** **이미지 카드 = 8종**(각진·둥근·원형·아치·구름·꽃·반짝이·마름모) / **카드 배경·지도·갤러리 = 각진(0px)·둥근(16px)·원형(999px) 3종**. 픽셀 임의값 ❌. *(2026-09-07 정정 — 옛 「3가지뿐」은 카드 배경 얘기였다)*
- **Primary:** 배경 = `{버튼색}`(채움), 텍스트 = `{배경색}`. 높이 ~58px.
- **금지:** 위치 안내 카드 버튼을 테두리만 있는 outline으로 만들지 말 것(채움 고정). 단일 SNS 강조는 버튼 카드(**SNS 카드는 2개 이상 묶음일 때 · 배치는 자유 — 「푸터 전용」은 오류, 2026-09-07 정정**).
...
- **메뉴:** **내/외부 이동 내비게이션**(페이지 이동·카드 앵커·외부 URL)이 본래 용도. default 액션(전화·문자·이메일·vCard) 묶음에도 적합하며 다른 용도도 가능.
- **2열 카드:** 2~4분할 등폭 FILL + 내부 텍스트/이미지/버튼/예약 삽입. 다중 버튼 그리드의 대안.
