...
```bash
B="$HOME/.Codex/skills/gstack/browse/dist/browse"   # gstack browse
DRV=".Codex/skills/cdbd-card-automation/card-driver.js"
...
```
- 라벨링 컨벤션(목적 위주)은 [[../AGENTS.md]] 4단계 🏷 카드 라벨 + [[1-5-1. CdBd 에디터#7. 카드 라벨 = 목적·내용 이름]]. 참고 예시: editor 4856(채용설명회) — "회사명/슬로건/여백/초대 메시지/위치 안내/출석 등록 예약" 등.
...
> 구방식(fallback): onChange가 안 통하는 픽커는 [[AGENTS.md]] "페이지 색상 — SketchPicker 실제클릭" 항목(색 원 `$B click` + hex `$B fill`). per-card 디자인 보드 색상도 동일 onChange 구조 추정(미검증).
...
- 식별은 **라벨 아님, fiber `block.type`** ([[AGENTS.md]] SVG 시그니처 원칙과 동일 취지: DOM 텍스트로 추정 금지).
