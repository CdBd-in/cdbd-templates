...
## 🔁 2026-09-15 재대조 — 어드민 354 ↔ 색인 355

전수 대조(어드민 키 354개 ↔ `preset-index.json` 355개) 결과 **차이는 1건뿐**이다.

| 이름 | Figma | 서랍 | 담는 칸 | 상태 |
|---|---|---|---|---|
| `list-mixed-27` | `6560:8949` (h116 · 380×116) | 나열-미에볼 | 메뉴1~6 (2열 그리드 버튼) | ⛔ **어드민 미등록** |

- 색인·Figma에는 **실재**한다(옛 이름 `16-V3 2열 그리드 버튼`). 없어진 섹션이 아니라 **아직 프리셋으로 저장되지 않은 것**.
- 등록은 어드민이 아니라 **에디터 「프리셋 설정 → 프리셋 저장」**에서만 된다 → 에디터에 이 섹션을 만든 뒤 `sec/mixed/list/27`로 저장하면 354 → 355로 맞는다.
- 그 밖 **353개는 키·이름이 전부 일치**(삭제·유실 0건).

## 🔤 2026-09-15 — 「안내 사항」 띄어쓰기 통일

어드민 프리셋 화면 표기가 정본(`안내 사항`)이라, **Figma 그룹 이름**을 맞췄다.

| 대상 | 개명 |
|---|---|
| SECTION `5455:1193` | `안내사항 (유형별 분류)` → **`안내 사항 (유형별 분류)`** |
| FRAME `5455:1227`·`5455:1242`·`5455:1198`·`5455:1213`·`5455:1194` | `안내사항-{스타일}` → **`안내 사항-{스타일}`** |
| `preset-index.json` | `grp` 16건 갱신 (`old_name`은 이력이라 보존) |
| `rebuild-picklist.py` | `ORDER` 목록의 `안내사항` → `안내 사항` (안 고치면 픽리스트에서 맨 뒤로 밀림) |

→ **어드민 · Figma · 문서 · 색인 네 곳이 같은 이름.** 목적 이름을 다시 손대면 네 곳을 함께 고칠 것.
